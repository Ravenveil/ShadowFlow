...
def _default_runtime_root() -> Path:
    override = os.environ.get("AGENTGRAPH_RUNTIME_ROOT")
    if override:
        return Path(override).expanduser()
    codex_memories = Path.home() / ".codex" / "memories"
    if codex_memories.exists():
        return codex_memories / "agentgraph-runtime"
    return Path.cwd() / "agentgraph-runtime"
...
def _build_runtime_service(
    ...
    if store == "memory":
        ...
        from agentgraph.runtime.checkpoint_store import ZeroGCheckpointStore  # imported lazily
        url = bridge_url or os.environ.get("AGENTGRAPH_BRIDGE_URL", "http://localhost:3001")
...
async def init_workflow_command(
    ...
    with tempfile.TemporaryDirectory(prefix="agentgraph-init-workflow-") as temp_root:
...
async def chat_with_executor(args) -> int:
    ...
    print(f"[AgentGraph chat] session={session.session.session_id} provider={args.provider}")
    print("输入 /exit 结束对话。")
    while True:
        try:
...
def main():
    ...
    run_parser.add_argument(
        '--bridge-url',
        default=None,
        help='0G Node.js bridge URL (for --store zerog, overrides AGENTGRAPH_BRIDGE_URL)',
    )
    ...
    registry_list_parser.add_argument('--kind', required=True, choices=['tools', 'skills', 'roles', 'agents', 'templates'], help='Registry kind')
    ...
    registry_get_parser.add_argument('--kind', required=True, choices=['tools', 'skills', 'roles', 'agents', 'templates'], help='Registry kind')
    ...
    chat_parser.add_argument('--title', default='AgentGraph Chat Session', help='Chat session title')
    ...
    if args.command == 'validate':
        ...
        if not workflow_path:
            run_parser.error("workflow file required: use 'agentgraph run workflow.yaml' or '-w workflow.yaml'")
        if not args.input:
