...
Exposes AgentGraph workflow execution as an MCP tool so Claude Code,
...
TOOLS = [
    {
        "name": "run_workflow",
        "description": (
            "Run an AgentGraph YAML workflow end-to-end. "
...
async def _handle_request(request: Dict) -> Dict | None:
    ...
    if method == "initialize":
        return _ok(id_, {
            ...
            "capabilities": {"tools": {}},
            "serverInfo": {"name": "agentgraph", "version": "0.1.0"},
        })
...
async def run_http(host: str = "127.0.0.1", port: int = 3002) -> None:
    ...
    print(f"[agentgraph mcp] HTTP debug server on http://{host}:{port}", file=sys.stderr)
    httpd.serve_forever()
