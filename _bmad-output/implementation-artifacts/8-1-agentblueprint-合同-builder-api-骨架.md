...
**And** `RoleProfile` 支持**主管-员工层级结构**：
- `sub_agents: list[RoleProfile] = []` — 该角色管理的员工 Agent 列表（可递归嵌套）
- `can_spawn_tasks: bool = False` — 是否是主管角色；为 `True` 时 Executor 自动为其开启 `builtin:spawn_task` 工具
- 主管的 `sub_agents` 不为空时，`can_spawn_tasks` 默认升为 `True`（`model_validator` 自动推断）
- 员工 Agent 的 `sub_agents` 默认为空，`can_spawn_tasks` 默认 `False`
- `instantiate_blueprint()` 映射时，主管 → 工作流 Orchestrator 节点，员工 → 其动态可调度的子节点池
