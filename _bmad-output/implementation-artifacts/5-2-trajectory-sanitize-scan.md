...
- [ ] **AC1 — sanitize 引擎实现**
  - [ ] 新建 `shadowflow/runtime/sanitize.py`,包含:
  ...
  - [ ] `sanitize_trajectory(trajectory: dict) -> (cleaned, removed: list[RemovedField])`
  - [ ] `RemovedField` schema:`{path: "messages[2].content", pattern: "email", sample_masked: "j***@g***.com"}`(不返回原值,只返回脱敏示例)
  - [ ] 黑名单补丁:字段名为 `private_key` / `api_key` / `password` / `authorization` 整字段直删
- [ ] **AC1 — API 端点**
  - [ ] `shadowflow/api/workflow.py` 新增 `POST /workflow/runs/{id}/trajectory/sanitize`
  - [ ] 输入:`{trajectory: dict}`;输出:`{cleaned_trajectory, removed_fields: [...], had_matches: bool}`
  - [ ] 纯计算,无副作用,不写库
- [ ] **AC2 — 前端确认流程**
  - [ ] `src/pages/EditorPage.tsx` 在 "Publish to 0G" 按钮点击时先调 sanitize
  - [ ] 若 `had_matches=true` → 弹 Modal 展示 `removed_fields` 表格(字段路径 / 类型 / 脱敏示例)
  - [ ] Modal 两个按钮:`确认继续上传` / `取消`,默认焦点在"取消"(S2 非静默:默认保守)
  - [ ] `had_matches=false` → 直接跳过 Modal,进入 Story 5.1 上传流程
- [ ] **单元测试**
  - [ ] 新建 `tests/test_sanitize.py`,覆盖:
