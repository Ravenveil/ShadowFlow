  ...
  return (
    <>
      ...
      <div style={{
        ...
        {loadState === 'loading' && (
          <div style={{ padding: '24px 16px', fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--fg-4)', textAlign: 'center' }}>
            加载 Teams 中…
          </div>
        )}
        ...
        {loadState === 'ok' && teams.length === 0 && (
          <div
            ...
            style={{ padding: '24px 16px', fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--fg-4)', textAlign: 'center' }}
          >
            还没有 Team — 点 + 创建
          </div>
        )}
            ...
            {teams.map((t, i) => (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, width: '100%' }}>
                  ...
                  <span style={{ fontSize: 13, fontWeight: i === selectedIdx ? 700 : 600, color: 'var(--fg-1)', flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                    {t.name}
                  </span>
                  <span style={{
                    minWidth: 18, height: 16, padding: '0 5px', borderRadius: 8,
                    background: t.run ? 'var(--accent)' : 'var(--bg-elev-3)',
                    color: t.run ? 'var(--skin-accent-ink)' : 'var(--fg-3)',
                    border: t.run ? 'none' : '1px solid var(--border)',
                ...
                {t.agent_ids.length > 0 && (
                  <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap', paddingLeft: 0 }}>
                      ...
                      return (
                        <span
                          ...
                          style={{
                            ...
                            fontSize: 8, fontWeight: 700, color: 'var(--fg-3)',
                    ...
                    {t.agent_ids.length > 5 && (
                      <span style={{ fontSize: 9, color: 'var(--fg-5)', alignSelf: 'center', fontFamily: 'var(--font-mono)' }}>
                        +{t.agent_ids.length - 5}
                      </span>
                    )}
                  </div>
        ...
        <div style={{
          ...
          fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--fg-4)', lineHeight: 1.6,
        }}>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span>Policy Matrix</span>
            <span style={{ color: 'var(--accent-bright)' }}>L2-strict</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
            <span>retry_gate</span>
            <span style={{ color: 'var(--fg-2)' }}>3</span>
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              {cur && !cur.run && (
                <span style={{
                  ...
                  color: 'var(--fg-4)', background: 'var(--bg-elev-2)',
                }}>{cur.status}</span>
              )}
              {cur && <FBPill color="var(--fg-4)" dim>POLICY · {cur.policy}</FBPill>}
            </div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--fg-4)', marginTop: 3, letterSpacing: '0.02em' }}>
              {cur
                ? (() => {
          ...
          <button
            ...
            style={{
              background: matrixView ? 'var(--accent-tint)' : undefined,
              color: matrixView ? 'var(--accent-bright)' : undefined,
              display: 'inline-flex',
              alignItems: 'center',
              gap: 5,
            }}
        ...
        <div className="fb-canvas" style={{ flex: 1, position: 'relative', overflow: 'hidden' }}>
          ...
          <svg style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', pointerEvents: 'none' }}>
            <defs>
              <marker id="dag-ar" viewBox="0 0 10 10" refX="9" refY="5" markerWidth="7" markerHeight="7" orient="auto">
                <path d="M0 0 L10 5 L0 10 z" fill="var(--fg-5)" />
              </marker>
          ...
          {dagLoading && (
            <div style={{
              ...
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--fg-4)' }}>加载工作流…</span>
            </div>
          )}
          ...
          {!dagLoading && cur && canvasNodes.length === 0 && (
            <div style={{
              ...
              <div style={{ fontSize: 13, color: 'var(--fg-3)', fontWeight: 600 }}>此 Team 暂无成员</div>
              <div style={{
          ...
          <div style={{
            ...
            background: 'var(--skin-panel)', border: '1px solid var(--border)', borderRadius: 8,
          }}>
            {[
            ...
            <span style={{ padding: '0 8px', fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--fg-4)', alignSelf: 'center' }}>100%</span>
          </div>
        </div>
      ...
      {!inspectorOpen && (
        <button
          ...
          style={{
            ...
            background: 'var(--bg-elev-2)', border: '1px solid var(--border)', borderRight: 'none',
            color: 'var(--fg-3)', cursor: 'pointer', fontFamily: 'var(--font-mono)', fontSize: 11, zIndex: 10,
          }}
        >‹</button>
      )}
            ...
            {selectedNode ? (
              <>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  <FBPill color={
                    selectedNode.status === 'running' ? 'var(--accent-bright)'
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  {[
                    ...
                    <div key={s.k} style={{
                      ...
                      border: '1px solid var(--border)', borderLeft: '2px solid var(--accent)', borderRadius: 8,
                    }}>
                      <div className="fb-label" style={{ fontSize: 9 }}>{s.k}</div>
                      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 18, fontWeight: 700, color: 'var(--fg-1)', marginTop: 2 }}>{s.v}</div>
                      <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--fg-4)', marginTop: 1 }}>{s.d}</div>
                    </div>
                  ))}
                </div>
                ...
                <div className="fb-card" style={{ padding: 12 }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr auto auto', gap: '6px 10px', fontFamily: 'var(--font-mono)', fontSize: 11 }}>
                      ...
                      .map((e: CanvasEdge, i: number) => {
                        ...
                        return (
                          <React.Fragment key={i}>
                            <span style={{ color: 'var(--fg-2)' }}>from {srcNode?.label ?? e.fromId.slice(-6)}</span>
                            <span style={{ color: 'var(--status-ok)' }}>auto</span>
                    ...
                    {canvasEdges.filter((e: CanvasEdge) => e.toId === selectedNode.id).length === 0 && (
                      <span style={{ color: 'var(--fg-5)', gridColumn: '1 / -1', fontSize: 10 }}>
                        无入边 (起始节点)
                      </span>
                    )}
                  </div>
                  ...
                  {cur?.run ? (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                      {[
                        ...
                        { t: '09:18', m: 'rollback to draft v2 · ckpt.saved',   c: 'var(--fg-3)' },
                        { t: '09:18', m: 'retry r2/3 · running...',              c: 'var(--accent)' },
                      ].map((h, i) => (
                        <div key={i} style={{
                          display: 'flex', gap: 8, alignItems: 'flex-start', padding: '7px 10px',
                          background: 'var(--bg-elev-2)', borderLeft: `2px solid ${h.c}`, borderRadius: 4,
                        }}>
                          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--fg-5)' }}>{h.t}</span>
                          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10.5, color: 'var(--fg-2)', flex: 1 }}>{h.m}</span>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div style={{
                      padding: '12px 10px', background: 'var(--bg-elev-2)',
                      borderRadius: 6, fontFamily: 'var(--font-mono)', fontSize: 10,
                      color: 'var(--fg-5)', textAlign: 'center', lineHeight: 1.7,
                    }}>
      ...
      {dagEditorOpen && (
        ...
        >
          <div style={{ width: 640, background: 'var(--skin-panel)', border: '1px solid var(--border)', borderRadius: 10, boxShadow: 'var(--shadow-pop)', padding: 20 }}>
            ...
            <div style={{ background: 'var(--bg-elev-1)', border: '1px solid var(--border)', borderRadius: 8, padding: 14, fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--fg-3)', lineHeight: 1.7, marginBottom: 14, maxHeight: 320, overflow: 'auto' }}>
              ...
              {!dagLoading && dagData && (
                <>
                  ...
                  {dagData.nodes.length === 0 && dagData.edges.length === 0 && (
                    <div style={{ color: 'var(--fg-5)', fontStyle: 'italic' }}>
                      该 Team 暂无 workflow 配置 · PUT /api/teams/{cur?.team_id}/workflow 上传节点 + 连线
                    </div>
                  )}
                  {dagData.nodes.map(n => (
                    <div key={n.id}>· <span style={{ color: 'var(--accent-bright)' }}>{n.data.name}</span> ({n.type}) @ ({n.position.x},{n.position.y})</div>
                  ))}
                  {dagData.edges.length > 0 && <div style={{ marginTop: 6, color: 'var(--fg-4)' }}>—— edges ——</div>}
                  {dagData.edges.map(e => (
      ...
      {matrixView && (
        ...
        >
          <div style={{ width: 560, background: 'var(--skin-panel)', border: '1px solid var(--border)', borderRadius: 10, boxShadow: 'var(--shadow-pop)', padding: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              ...
              <span style={{ flex: 1 }} />
              <span onClick={() => setMatrixView(false)} style={{ cursor: 'pointer', color: 'var(--fg-4)', fontFamily: 'var(--font-mono)', fontSize: 11 }}>ESC</span>
            </div>
            <div style={{ background: 'var(--bg-elev-1)', border: '1px solid var(--border)', borderRadius: 8, padding: 14, maxHeight: 320, overflow: 'auto' }}>
              {policyLoading && <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--fg-3)' }}>加载中…</div>}
              {!policyLoading && policyData && (() => {
                const rowKeys = Object.keys(policyData);
                if (rowKeys.length === 0) {
                  return <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--fg-5)', fontStyle: 'italic' }}>
                ...
                return (
                  <table style={{ fontFamily: 'var(--font-mono)', fontSize: 10.5, borderCollapse: 'collapse', width: '100%' }}>
                    <thead>
                      <tr><th style={{ textAlign: 'left', padding: '4px 8px', color: 'var(--fg-4)' }}>sender ↘ receiver</th>
                        {colKeys.map(c => <th key={c} style={{ padding: '4px 8px', color: 'var(--fg-4)' }}>{c}</th>)}
                      </tr>
                    </thead>
                    <tbody>
                      {rowKeys.map(r => (
                        <tr key={r}>
                          <td style={{ padding: '4px 8px', color: 'var(--fg-2)', fontWeight: 600 }}>{r}</td>
                          {colKeys.map(c => {
                            const v = policyData[r]?.[c];
                            return <td key={c} style={{ padding: '4px 8px', textAlign: 'center', color: v ? cellColor[v] : 'var(--fg-5)' }}>{v ?? '—'}</td>;
                          })}
                        </tr>
                      ))}
                    </tbody>
        ...
        >
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <div>
                <label style={{ fontFamily: 'var(--font-mono)', fontSize: 11, color: 'var(--fg-3)', display: 'block', marginBottom: 4 }}>
                  Team name
                </label>
                <input
                  data-testid="ct-name"
