/**
 * TeamEditChat — 对话式 Team 编辑器
 *
 * 嵌入 TabTeams 中央面板底部，用户用自然语言描述修改，LLM 解析后自动更新 Team 配置。
 */

import { useState, useRef, useEffect } from 'react';
import { FBIcons } from './FBAtoms';
import { chatEditTeam, type TeamChatResponse } from '../../api/teams';
...
export function TeamEditChat({ teamId, onTeamUpdated }: TeamEditChatProps) {
  ...
  const handleSend = async () => {
    ...
    try {
      const resp: TeamChatResponse = await chatEditTeam(teamId, msg, key, provider);
  ...
  if (!expanded) {
    return (
      ...
      >
        ...
        <span style={{ fontSize: 12, color: 'var(--fg-4)' }}>对话修改 Team ... 说"改名为..."、"加一个 agent"、"改描述"</span>
      </div>
    );
  }
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <span style={{ width: 14, height: 14, display: 'flex', color: 'var(--accent-bright)' }}>{FBIcons.chat}</span>
        <span style={{ fontSize: 11, fontWeight: 700, color: 'var(--fg-2)' }}>对话修改</span>
        <span style={{ flex: 1 }} />
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--fg-5)' }}>用自然语言修改 Team 配置</span>
        <button
      ...
      <div style={{
        ...
        <div style={{
          ...
          <textarea
            ...
            placeholder="改名为研发小队、加 agent-003 到团队..."
            disabled={sending}
