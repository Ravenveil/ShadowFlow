...
export const DayDivider = memo(function DayDivider({ label }: { label: string }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, margin: '4px 0' }}>
      ...
      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'var(--fg-4)', padding: '2px 10px', borderRadius: 10, background: 'var(--bg-elev-1)', border: '1px solid var(--border)', fontWeight: 600 }}>{label}</span>
      <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
    </div>
  );
});
...
export const SystemNote = memo(function SystemNote({ text }: { text: string }) {
  return (
    <div style={{ textAlign: 'center' }}>
      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--fg-4)', padding: '4px 10px', background: 'var(--bg-elev-1)', borderRadius: 6, border: '1px dashed var(--border)' }}>{text}</span>
    </div>
  );
});
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center' }}>
      <div style={{
        ...
        background: tone === 'reject' ? 'color-mix(in oklab, var(--status-reject) 10%, var(--bg-elev-1))' : 'var(--bg-elev-1)',
    ...
    <div style={{ display: 'flex', gap: 10, position: 'relative' }} className="fb-msg">
      ...
      <div style={{ flex: 1, minWidth: 0, maxWidth: 680, borderLeft: `2px solid ${agent.color}`, paddingLeft: 11 }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 7, marginBottom: 3, flexWrap: 'wrap' }}>
          ...
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'var(--fg-4)' }}>{agent.role}</span>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--fg-4)', padding: '1px 5px', borderRadius: 3, background: 'var(--bg-elev-2)', border: '1px solid var(--border)' }}>{agent.model}</span>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'var(--fg-5)' }}>{time}</span>
          {edited && <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--fg-5)' }}>· 已编辑</span>}
        </div>
        <div style={{ fontSize: 12.5, lineHeight: 1.55, color: 'var(--fg-2)' }}>{body}</div>
        {memoriesRecalled != null && memoriesRecalled > 0 && (
          <MemoryRecallRow memories={memoriesRecalled} />
        )}

        {tool && (
          <div style={{ marginTop: 7, display: 'inline-flex', alignItems: 'center', gap: 8, padding: '5px 9px', borderRadius: 6, background: 'var(--bg-elev-2)', border: '1px solid var(--border)' }}>
            <span style={{ width: 11, height: 11, display: 'flex', color: 'var(--fg-4)' }}>{CI.clip}</span>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--fg-2)', fontWeight: 600 }}>{tool.name}</span>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--fg-4)' }}>{tool.meta}</span>
          ...
          <div style={{ display: 'flex', gap: 5, marginTop: 7, flexWrap: 'wrap' }}>
            {reactions.map(([emo, n], i) => (
              <span key={emo} onClick={() => onAddReaction?.(msgId, emo)} style={{
                ...
                background: i === 0 ? 'var(--accent-tint)' : 'var(--bg-elev-2)',
                border: `1px solid ${i === 0 ? 'color-mix(in oklab, var(--accent) 35%, transparent)' : 'var(--border)'}`,
                fontSize: 11, color: i === 0 ? 'var(--accent-bright)' : 'var(--fg-3)',
            ...
            <span onClick={() => onAddReaction?.(msgId, '👍')} style={{
              ...
              borderRadius: 11, background: 'var(--bg-elev-2)', border: '1px dashed var(--border-strong)',
        ...
        {thread && (
          <div onClick={onThreadOpen} style={{
            ...
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--fg-4)' }}>最后 {thread.last}</span>
            <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10 }}>›</span>
          </div>
        )}

        {readBy && <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--fg-5)', marginTop: 5 }}>{readBy}</div>}
      </div>
    ...
    <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
      <div style={{ maxWidth: 560, display: 'flex', flexDirection: 'column', alignItems: 'flex-end' }}>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 7, marginBottom: 3 }}>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'var(--fg-5)' }}>{time}</span>
          <span style={{ fontSize: 12.5, fontWeight: 700, color: 'var(--fg-1)' }}>{name}</span>
        </div>
        {reply && (
          <div style={{
            fontSize: 11, padding: '4px 9px', borderRadius: 5, marginBottom: 4,
            background: 'var(--bg-elev-2)', borderLeft: '2px solid var(--fg-5)',
            color: 'var(--fg-4)', maxWidth: '100%',
          }}>
            <span style={{ fontWeight: 700, color: 'var(--fg-3)' }}>{reply.name}</span>
            <span style={{ marginLeft: 6 }}>{reply.text}</span>
          </div>
        )}
        <div style={{
          padding: '8px 12px', borderRadius: '12px 4px 12px 12px',
          background: 'color-mix(in oklab, var(--status-ok) 14%, var(--bg-elev-1))',
          border: '1px solid color-mix(in oklab, var(--status-ok) 30%, transparent)',
          fontSize: 12.5, lineHeight: 1.5, color: 'var(--fg-1)',
        }}>{body}</div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--fg-5)', marginTop: 3 }}>已送达 · 5/5 已读</div>
      </div>
  ...
  return (
    <div style={{ display: 'flex', justifyContent: 'center', margin: '4px 0' }} data-testid="gate-pending">
      <div style={{
        ...
        background: 'color-mix(in oklab, var(--accent) 7%, var(--bg-elev-1))',
        border: '1px solid color-mix(in oklab, var(--accent) 40%, transparent)',
        boxShadow: 'var(--glow-accent)', position: 'relative',
      }}>
        <div style={{ position: 'absolute', top: -10, left: 14, padding: '2px 9px', background: 'var(--accent)', color: 'var(--accent-ink)', borderRadius: 4, fontFamily: 'var(--font-mono)', fontSize: 9.5, fontWeight: 800, letterSpacing: '0.08em' }}>⚑ APPROVAL GATE</div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
          <FBAv glyph="写" color="#EF4444" size={26} square />
          <span style={{ fontSize: 12, fontWeight: 700 }}>小写</span>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'var(--fg-4)' }}>WRITER · L3 · gate</span>
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'var(--fg-5)', marginLeft: 'auto' }}>09:21 · 等待 1m04s</span>
        </div>
        <div style={{ fontSize: 12.5, color: 'var(--fg-2)', marginBottom: 8, lineHeight: 1.5 }}>
          重写 §6 完成。新增 <span style={{ fontFamily: 'var(--font-mono)', color: 'var(--accent-bright)' }}>RetroCorr</span> 基线对比 + 联合消融表。请审审过目。
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6, marginBottom: 9 }}>
          {[['diff', '+142 / -38'], ['tokens', '2.1k / 5k'], ['retry', 'r2/3']].map(([k, v], i) => (
            <div key={i} style={{ padding: '5px 8px', background: 'var(--bg-elev-2)', borderRadius: 5, border: '1px solid var(--border)' }}>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--fg-4)', letterSpacing: '0.06em', textTransform: 'uppercase' }}>{k}</div>
              <div style={{ fontFamily: 'var(--font-mono)', fontSize: 11.5, fontWeight: 700, color: 'var(--fg-1)' }}>{v}</div>
            </div>
          ))}
        </div>
        <div style={{ display: 'flex', gap: 7, alignItems: 'center' }}>
          ...
          <span style={{ marginLeft: 'auto', fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'var(--fg-4)' }}>Y / N · ⌘↵ 批准</span>
        </div>
      </div>
      ...
      {diffOpen && (
        <div onClick={() => setDiffOpen(false)} style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            width: 720, maxHeight: '80vh', background: 'var(--skin-panel)', border: '1px solid var(--border)',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
              ...
              <span style={{ flex: 1 }} />
              <span onClick={() => setDiffOpen(false)} style={{ cursor: 'pointer', color: 'var(--fg-4)', fontFamily: 'var(--font-mono)', fontSize: 11 }}>ESC</span>
            </div>
            <div style={{ flex: 1, overflow: 'auto', background: 'var(--bg-elev-1)', border: '1px solid var(--border)', borderRadius: 8, padding: 14, fontFamily: 'var(--font-mono)', fontSize: 11.5, lineHeight: 1.6 }}>
...
export function TypingIndicator() {
  return (
    <div style={{ display: 'flex', gap: 10, alignItems: 'center', paddingLeft: 2 }}>
      ...
      <span style={{ fontSize: 11.5, color: 'var(--fg-4)' }}>审审 正在思考</span>
      <span style={{ display: 'flex', gap: 3 }}>
        {[0, 1, 2].map(i => (
          <span key={i} style={{
            width: 5, height: 5, borderRadius: '50%', background: 'var(--fg-4)',
            animation: `fb-pulse 1.2s ease-in-out ${i * 0.18}s infinite`,
          }} />
        ))}
      </span>
      <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'var(--fg-5)', marginLeft: 6 }}>~480 tokens · ~3.2s</span>
    </div>
  );
}
