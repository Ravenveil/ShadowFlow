...
function InboxRow({ id, hash, lock, n, desc, u, active, run, warn, t, members, mention, onClick }: InboxRowProps) {
  return (
    <div className={`fb-row ${active ? 'active' : ''}`} onClick={onClick} data-testid={`inbox-${id}`} style={{
      ...
      <span style={{
        width: 32, height: 32, borderRadius: 7, background: 'var(--bg-elev-3)', border: '1px solid var(--border)',
        ...
        {run && <span style={{ position: 'absolute', right: -2, bottom: -2, width: 9, height: 9, borderRadius: '50%', background: 'var(--status-run)', border: '2px solid var(--bg-elev-1)', animation: 'fb-pulse 1.4s infinite' }} />}
        {warn && !run && <span style={{ position: 'absolute', right: -2, bottom: -2, width: 9, height: 9, borderRadius: '50%', background: 'var(--status-warn)', border: '2px solid var(--bg-elev-1)' }} />}
      </span>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
          <span style={{ fontSize: 12.5, fontWeight: active || u > 0 ? 700 : 600, color: 'var(--fg-1)', flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{n}</span>
          {members !== undefined && <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9, color: 'var(--fg-5)' }}>{members}</span>}
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: u > 0 ? 'var(--accent-bright)' : 'var(--fg-5)' }}>{t}</span>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginTop: 2 }}>
          ...
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--fg-4)', flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{desc}</span>
        </div>
      </div>
  ...
  return (
    <div className={`fb-row ${active ? 'active' : ''}`} onClick={onClick} data-testid={`inbox-${id}`} style={{ padding: '7px 10px', borderRadius: 6 }}>
      <span style={{ position: 'relative' }}>
        ...
        {run && <span style={{ position: 'absolute', right: -2, top: -2, width: 8, height: 8, borderRadius: '50%', background: 'var(--status-run)', border: '1.5px solid var(--bg-elev-1)', animation: 'fb-pulse 1.4s infinite' }} />}
      </span>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
          <span style={{ fontSize: 12, fontWeight: 600, color: 'var(--fg-1)', flex: 1, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{n}</span>
          ...
          <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'var(--fg-5)' }}>{t}</span>
        </div>
        <div style={{ fontFamily: 'var(--font-mono)', fontSize: 10, color: 'var(--fg-4)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', marginTop: 1 }}>{last}</div>
      </div>
    </div>
  );
}
...
export function ChatInbox({ activeConv, setActiveConv }: { activeConv: ConvId; setActiveConv: (id: ConvId) => void }) {
  ...
  return (
    <div style={{ width: 268, borderRight: '1px solid var(--border)', background: 'var(--bg-elev-1)', display: 'flex', flexDirection: 'column', minHeight: 0 }}>
      <OrgSwitcher />
      ...
      <div style={{ padding: '4px 12px 8px' }}>
        <div className="fb-input" style={{ height: 30 }}>
          ...
          <span style={{ color: 'var(--fg-4)', fontSize: 11.5 }}>搜索 / 跳转</span>
          <span style={{ marginLeft: 'auto', fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'var(--fg-5)' }}>⌘F</span>
        </div>
      </div>
      ...
      <div style={{ padding: '0 12px 8px', display: 'flex', gap: 5, flexWrap: 'wrap' }}>
        {(['全部', '未读·12', '@我·3', 'Agent'] as const).map((l, i) => (
          <span key={i} onClick={() => setFilter(i)} data-testid={`filter-${i}`} style={{
            ...
            background: i === filter ? 'var(--accent-tint)' : 'var(--bg-elev-2)',
            color: i === filter ? 'var(--accent-bright)' : 'var(--fg-3)',
            border: `1px solid ${i === filter ? 'color-mix(in oklab, var(--accent) 35%, transparent)' : 'var(--border)'}`,
      ...
      <div className="fb-label-row" style={{ padding: '10px 14px 4px' }}>
        ...
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'var(--fg-4)' }}>3</span>
      </div>
      <div style={{ padding: '0 6px', display: 'flex', flexDirection: 'column' }}>
      ...
      <div className="fb-label-row" style={{ padding: '10px 14px 4px' }}>
        ...
        <span style={{ fontFamily: 'var(--font-mono)', fontSize: 9.5, color: 'var(--fg-4)' }}>4</span>
      </div>
      <div style={{ padding: '0 6px', display: 'flex', flexDirection: 'column' }}>
