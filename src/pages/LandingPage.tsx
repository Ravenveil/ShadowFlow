// ⚠️ UI PROTECTION: 只能加，不能删。
// 所有 section（Hero / VS Rivals / Features / Proof Wall / Templates / Import CID / CTA / Footer）
// 必须保留。可以改进样式、新增内容，但禁止删除或缩减任何已有区块。
// 参考设计：git commit b35001e (src/App.tsx)
...
type Lang = 'EN' | 'CN';
// Loose row types — the EN/CN literal arrays carry per-row optional flags
// (up / mono / grad) which `as const` would lock to never-ish unions. We type
...
function ChainMini({ steps }: { steps: readonly string[] }) {
  return (
  ...
  return (
    <>
      <Helmet>
        ...
        <meta name="description" content="链上可验证的 AI 多 Agent 协作平台，团队即资产" />
        <meta property="og:title" content="ShadowFlow — AI 协作团队即链上资产" />
      ...
      <div style={{ minHeight: '100vh', background: 'var(--t-bg)', overflowX: 'hidden' }}>
        ...
        <div style={{ position: 'relative', maxWidth: 1440, margin: '40px auto 0', padding: '60px 40px', textAlign: 'center', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', gap: 10, justifyContent: 'center', position: 'relative', zIndex: 2 }}>
            ...
            <a href={GITHUB_URL} target="_blank" rel="noopener noreferrer" className="sf-btn sf-btn-ghost" style={{ height: 52, padding: '0 30px', fontSize: 14 }}>{t.ctaBandSecondary}</a>
          </div>
        </div>
